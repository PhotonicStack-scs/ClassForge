...
public class ConstraintPropagationTests
{
    ...
    public void DomainReduction_BlockedSlotsRemoved()
    {
        ...
        var blockedSlotId = input.SchoolDays[0].TimeSlots[0].Id;
    ...
    public void DomainReduction_UnqualifiedTeachersExcluded()
    {
        ...
        var unqualifiedTeacher = new SchedulingTeacher(
            ...
            [], // no qualifications
            input.SchoolDays.Select(d => new SchedulingTeacherDayConfig(d.Id, 5)).ToList(),
            []);
    ...
    [Fact]
    public void YearDayConfig_SlotsExceedingMaxPeriods_Excluded()
    {
        var input = TestDataBuilder.CreateMinimalInput(
            yearCount: 1, classesPerYear: 1, subjectCount: 1,
        ...
        var modifiedInput = input with
        {
            YearDayConfigs = input.YearDayConfigs
                .Select(ydc => ydc with { MaxPeriods = 2 }).ToList()
        };

        var state = ConstraintPropagation.BuildInitialState(modifiedInput);

        var allSlots = modifiedInput.SchoolDays.SelectMany(d => d.TimeSlots).ToList();
        foreach (var variable in state.Variables)
        {
